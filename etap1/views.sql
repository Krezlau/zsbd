-- szczegóły zawodników
CREATE VIEW Transfers.v_Current_Players_Details AS
SELECT
    P.id AS Player_ID,
...
-- szczegóły klubów
CREATE VIEW Transfers.v_Club_League_Coach_Details AS
SELECT
    CL.id AS Club_ID,
...
-- szczegóły transferów
CREATE VIEW Transfers.v_Transfer_History AS
SELECT
    T.id AS Transfer_ID,
...
-- statystyki zawodników w sezonie 
CREATE VIEW Transfers.v_Player_Season_Stats AS
SELECT
    S.id AS Stat_ID,
    P.first_name AS Player_First_Name,
    P.last_name AS Player_Last_Name,
...
-- aktualni agenci zawodników
CREATE VIEW Transfers.v_Current_Player_Agents AS
SELECT
    P.first_name AS Player_First_Name,
    P.last_name AS Player_Last_Name,
...
WHERE
    ...
    OR PA.contract_end_date >= GETDATE();
GO

select * from Transfers.v_Current_Player_Agents